...
- **候选路线（调研后更新）**：不再把 `Qwen3.5-4B + Qwen3.5-9B` 冻结为双模型方案。第一轮按同一 ProfAgent 数据集评测：①成本基线 `Qwen3.5-9B`；②速度/质量主候选 `Qwen3.6-35B-A3B-FP8`；③质量上限候选 `Qwen3.6-27B-FP8`。三个 Qwen 候选均必须以包含 Vision Encoder 的原生多模态形态部署；`GLM-4.7-Flash` 只可作文本侧参考，因不满足单一多模态主服务合同而不进入生产主模型候选；`Qwen3.5-4B` 只保留为最低成本下界，不作为默认生产模型。候选名称均为评测起点，未取得本项目真实延迟、质量和安全证据前不得冻结唯一生产模型。
