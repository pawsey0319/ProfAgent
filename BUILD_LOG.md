...
- **Ruling Q 实际单次结果（2026-08-25，fail-closed 终止）**：docs commit `5a6c37e` 后 tester 与第二位 fresh reviewer 将 HEAD、唯一逐字命令、诊断脚本 blob `22c2b3b6ef42ec1399a888993af23af0101a7217`、六证据/config/marker/process 重新绑定并签收 `PASS`、`P0/P1/P2=0/0/0`。唯一命令随后只执行一次，约 2.79 秒即以 exit `1` 在脚本 top-level `from profagent.config import Settings` 处触发 `ModuleNotFoundError: No module named 'profagent'`；未生成合法 6+3 JSON。因为 `VisionAdapter` 尚未导入或构造，实际 network/CPA/Image/Vision/Provider calls 均为 `0`。supervisor 立即停止且没有重试；事后六证据、HEAD/status/diff/marker/process 全部 exact。post-exec reviewer 返回 `FAIL`、`P0/P1/P2=0/1/0`：preflight 遗漏逐字 direct-script 命令的入口可导入性。Q 的单次授权已消耗，不能复用。
- **Ruling R direct-script 入口修复（2026-08-25，mock/offline 已关闭）**：tester `5c36dae` 以仓库根 cwd、torch128 当前 `sys.executable` 的真实 subprocess 冻结 direct-script 入口；受控错误 HEAD 必须在 Settings/Vision/Provider 前返回 exit `1`、空 stderr 和严格 `input_rejected` 6+3 JSON，并保证 reports/pyc/cache/六证据/marker/status 前后相等。RED 为 `1 failed, 58 deselected`，唯一失败即 Q 的 line 20 import error；旧 mock `58 passed`。backend `bbf6541` 只在 `profagent` imports 前从 resolved `__file__` 计算固定 repo `ROOT`、缺失时插入 `sys.path` 并复用原变量，未改 VisionAdapter/ingestion/evidence/输出合同；新增 subprocess `1 passed`、diagnostic `59 passed`。最终 tester gate：O `28/28`、licensed-assets `427 passed`、full `972 passed, 1 skipped in 356.45s`，base validator `3/50/20/50/30`，S16 `120`、owners `72/24/24`、类别 `24/24/16/18/18/10/10`；两份报告恢复到 JSON 153,322 bytes / `8c6134687e28aead91432bae9c6b696aeb6e51284df8ed2f2a3eba42588ddd36` 与 MD 11,191 bytes / `a4efe9a9bb0f11822792db798723643b8d64660fa9f6b59342b301c56ddb683e`。fresh reviewer 签收 `PASS`、`P0/P1/P2=0/0/0`，Q 的 importability P1 为 `ADDRESSED`；六证据 exact、marker absent、process0、diff clean、仅预期 manifest/sources 未跟踪，整个 Ruling R 为 0 external。该 ruling 只允许另立新 full-HEAD-bound read-only preflight，不授权真实诊断、retry、Image、ingestion、promotion、batch 或 provider follow-up。
- **Ruling S 单次 Vision-only 实际诊断（2026-08-25，fail-closed 已终止）**：Ruling R docs HEAD `fd95108e365377cd2f8a8e63d339eac70ce77608` 经 tester 与 fresh reviewer 重新只读绑定，双方均 `PASS`、`P0/P1/P2=0/0/0`；wrong-head direct-script subprocess `1 passed, 58 deselected`，script/test blobs、六证据、config/model、marker/process/status exact。唯一授权命令随后 exactly once 执行，约 6.98 秒、exit `1`；最小 stdout 为 `reason_code=response_schema_invalid`、`schema_stage=envelope`、`envelope_failure_code=unsupported_envelope_fields`、`envelope_metadata_profile=cpa_chat_completion_metadata_v1`、provenance `requested_model=grok4.6/resolved_model=null/model_verified=false`、`vision_call_count=1`，conda stderr 仅报告命令失败。supervisor 立即 STOP 且无重试。actual-run fresh reviewer 签收 `PASS`、`P0/P1/P2=0/0/0`：该状态可达且诚实，表示 CPA 返回了不符合当前闭合 Chat Completions envelope 合同的响应，诊断器在模型来源验证与图像语义评估前安全拒绝；不能解释为模型内容失败、Grok 4.6 已验证成功或 Vision 图片链路成功。输出未泄漏 raw response、Provider prose、凭据、路径或内部 ID；本轮 Image/retry/write/ingestion/promotion/batch 均为 `0`，六证据/HEAD/status/diff/marker/process 事后保持 exact，未生成、摄取或晋升任何资产。当前不得重试或继续 Provider follow-up，S16 图片链路仍未完成。
- **Ruling T CPA envelope 合同对齐调查（2026-08-25，`BLOCKED`、未进入 TDD）**：systematic-debugging Phase 1–3 只读调查确认，Ruling S 的 `response_schema_invalid / envelope / unsupported_envelope_fields` 由同一个合并 predicate 覆盖“非 object、缺 `model`、缺 `choices`、存在额外 top-level key”，因此 actual S symptom 不足以定位具体字段、层级、类型或 serializer 路径；项目 mock 中的 `usage/logprobs/audio` 仅证明本地拒绝策略，不是 CPA 实际字段证据。本机运行物可绑定为 CLIProxyAPI `7.2.97`、commit `42f36b94e0805a9897c3aa3be46a2b124be0057e`、exe SHA `ec3c6648686e84e8fef62464d23dbe3d84cfd7d1efa57e69917c5671f5a7a254`；残留 indexed pack 可证明该 commit 与完整 source tree，但关键 route serializer/test blobs 缺失，未索引 `tmp_pack_Ed8Ieq` 的 trailer checksum 无效，不能作为完整、可接受的源码快照。root-cause reviewer 返回 `ROOT CAUSE REVIEW FAIL`、`ENTER_TDD=NO`、`P0/P1/P2=1/1/1`；其中 P0 仅指“若基于猜测放宽 profile”会破坏 fail-closed 的风险，不是现有代码已发生 P0。Ruling T 全程 0 network/CPA/Vision/Image/diagnostic/ingestion 外调，0 生产代码、测试、profile 或 evidence 变化；unknown 继续 fail-closed。解除阻塞的唯一门槛是取得与 `42f36b94` 绑定、checksum-valid 且包含关键 route serializer 与 tests blobs 的本地源码 archive/worktree 或 `.pack + .idx`；此前不得写正向 compat RED、放宽 profile 或宣称合同对齐完成。
- **Ruling U 公开源码合同证据（2026-08-25，T 的证据阻塞已解除）**：经 `agent-reach`/GitHub REST 只读绑定公开仓库 `router-for-me/CLIProxyAPI`，commit `42f36b94e0805a9897c3aa3be46a2b124be0057e` 的 tree 精确为 `3787f8e6f984aa5f594e025778eddc40a810ff0f`；关键 blobs 为 XAI executor `2cbb41846bbd270ff111e6d37f8671c22dbe7dec`、Codex→OpenAI non-stream response `7feb9fdb55bfb231514c980bfea69f1878be5618`、对应 tests `663ee2334859f5f26b4240822ee47064c4f3e1d0`、OpenAI handler `cdb3c6c244fa57c30ebaee774bb4776253b6004e`、executor types `488db9c2d284989a0a0574ded3cad4a3781870a6`、translator registration `94db2a7db85f0ca0b6974f3071f5a20e79b00e9a`。可证默认 `/v1/chat/completions` 显式使用 OpenAI source/response format，XAI 将 request 从 OpenAI 转为 Codex，并在 `response.completed` 后调用 `TranslateNonStream(Codex, OpenAI)`；模板固定 `native_finish_reason`、`reasoning_content`、`tool_calls`，条件生成闭集 `usage`/`images`。首轮 root-cause review 唯一 P1 指出当前 v1 profile 也允许 serializer 不产生的 `system_fingerprint/refusal`，故两合同非等价且互不包含、不能写成 strict subset；ignored ledger 修正后 replacement review `PASS`、`ENTER_TDD=YES`、`P0/P1/P2=0/0/0`。该证据只证明版本化 schema-profile drift 与兼容目标；actual S 的具体字段、translator hooks 和部署配置仍未知，不得反推 `usage` 或宣称已观察任何字段。
- **Ruling U v2 profile mock-only TDD 收口（2026-08-25，已关闭）**：tester `f56da39` 以完整手写 wire envelope 冻结 `cpa_chat_completion_metadata_v2` 的已知 metadata validate→discard、strict types/closed nested keys/no-leak/unknown fail-closed，RED `45 failed, 427 deselected`；backend `5ef6fa5` 仅改 Vision，U `45 passed`。首轮 implementation reviewer 与 full gate 返回 `P0/P1/P2=0/3/0`、`1016 passed, 1 skipped, 1 failed`：v2 failure profile 尚未贯通 ingestion/diagnostic、stop-only 错收 `tool_calls=[]/array`、production 三元组破坏旧两元组 override seam。tester `4752e40` 冻结第二轮 RED（U `8 failed, 50 passed`、diagnostic U `5 failed, 6 passed`、legacy seam `1 failed`），backend `f468dc0` 只改 `profagent/vision.py`、ingestion 与 diagnostic：Vision stop-only 仅接受 `tool_calls=null`；两元组安全默认 v1；response-schema/model-mismatch 的 v1/v2 profile 在 public outcome/CLI/private v3/最小 6+3 diagnostic 中闭集传播，unknown/非法组合仍 fail-closed。replacement reviewer 将三个 P1 全部标为 `ADDRESSED`，最终 `PASS`、`P0/P1/P2=0/0/0`。独立 final gate：U `58 passed`、Ruling O exact `28 passed`、diagnostic `70 passed`、references `2 passed`、licensed-assets `485 passed`、full `1041 passed, 1 skipped in 301.15s`；base validator `3/50/20/50/30`，S16 validator `120`、owners `72/24/24`、类别 `24/24/16/18/18/10/10`，三文件 `py_compile` 通过；报告恢复为 JSON 153,322 bytes / `8c6134687e28aead91432bae9c6b696aeb6e51284df8ed2f2a3eba42588ddd36` 与 MD 11,191 bytes / `a4efe9a9bb0f11822792db798723643b8d64660fa9f6b59342b301c56ddb683e`。六证据仍精确为 `6e71d9ca9cd3e5e9c7a037c56498303ece03021179505dfd21ed2a5c218f4682`、`eca7fd576e10853388cb2de624698b06bb8a49bc8cb443e11e63118599ef19e2`、`0f981797ee2dc37d41a0563e1d5d34b8d31802ac5589beae4aa223309843dd28`、`46c5d710493b59daa823f72d9c68ab18d059e313ead69531c4cb70c2058a4e98`、`afc00f9705193d7d0ea2072989b07bdb6b74ceae757b1f20ba5f8575db662abf`、`33c9439e4516e571925689f0ef501a9771f3d25888ff24ecbe044c6b16d19749`；marker absent、relevant process `0`、tracked/cached clean，仅预期 manifest/sources 未跟踪。除公开 GitHub 只读取证外，整个 Ruling U 真实 CPA/Vision/Image/diagnostic/ingestion/provider 外调均为 `0`，没有生成、摄取或晋升资产；本 closure 不授权真实 preflight、diagnostic、retry 或 Provider 调用，S16 图片链路仍未完成。
- **Ruling V 单次 Vision-only 兼容验证（2026-09-07，actual-run 已关闭）**：在 HEAD `cf3016a24a0114e41b0ed7f3bdfff5495c28ef2c` 上，唯一授权命令 `conda run --no-capture-output -n torch128 python scripts/diagnose_cpa_vision_envelope.py --expected-head cf3016a24a0114e41b0ed7f3bdfff5495c28ef2c` exactly once 执行，exit `0`、exec wall `6.2841291s`；最小 stdout 为 `reason_code/schema_stage/envelope_failure_code=null`、`envelope_metadata_profile=cpa_chat_completion_metadata_v1`、provenance `requested_model=grok4.6/resolved_model=grok-4.6-build/model_verified=true`、`vision_call_count=1`。该 v1 profile 是诊断器对通过兼容层响应的归一化结果，不证明实际 wire response 出现或缺少某个 v1/v2 字段。事后只读审计确认六份冻结证据 hash/size exact、固定输入仍为 1024×1024 PNG、marker absent、相关 process `0`、tracked/cached clean，且状态仅含预期未跟踪 manifest/sources；g051 仍为 `quarantined`，无 license/path/accepted hashes/receipt，source g051 为 `0`。fresh actual-run reviewer 返回 `PASS`、`P0/P1/P2=0/0/0`、`DOC_UPDATE_ALLOWED=YES`。本次只证明当前 Vision 兼容验证成功；Image 调用、生图、图片质量、ingestion、promotion、g051 ready、S16B 与 S16 整体完成均未被证明，且 retry、Image、Provider follow-up、batch 或后续任何真实调用均未获授权。本文档提交会改变 HEAD，旧 HEAD 绑定不得用于后续外调。
