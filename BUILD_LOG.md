...
- **Ruling L（2026-08-24，仅授权、尚未执行）**：在 `39e9d4d` 上完成 read-only preflight：tracked tree clean，只有冻结的 manifest/source 未跟踪；四份 Ruling J 证据哈希精确不变；`canary-g051.jsonl` 恰好一行、SHA-256 `e21591fc33f27630ebd52dcd5fd8afb0b03f8aa9b12c5a1f487475dd416c936c`，且与 canonical g051 JSON 完全相等；CPA 静态配置绑定为 Text/Vision `grok4.6`（transport `grok-4.6-high`，reported allowlist `grok-4.6-high|grok-4.6-build`）和 Image `grok-imagine-image-quality`，Image/Text 均启用、API key 已配置、Vision failure injection 关闭。由此只授权使用冻结命令执行一次第三 g051 canary，最多一次 Image 和到达时一次 Vision；运行时模型仍必须由 Provider/adapter fail-closed 验证。不得添加 `--resume/--diagnostic-canary/--license/--source-map`，不得 retry、g052–g120、bulk、manual promote、Task 5/UI 或宣称 S16B 完成。执行一次后无论 ready/quarantined/failure 都必须立即停止、冻结新证据并交 fresh reviewer；本条不包含实际 CPA 调用。
