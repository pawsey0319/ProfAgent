...
- **候选路线（调研后更新）**：不再把 `Qwen3.5-4B + Qwen3.5-9B` 冻结为双模型方案。第一轮按同一 ProfAgent 数据集评测：①成本基线 `Qwen3.5-9B`；②速度/质量主候选 `Qwen3.6-35B-A3B-FP8`；③质量上限候选 `Qwen3.6-27B-FP8`；`GLM-4.7-Flash` 仅作中文 MoE 挑战者。`Qwen3.5-4B` 只保留为最低成本下界，不作为默认生产模型。候选名称均为评测起点，未取得本项目真实延迟、质量和安全证据前不得冻结唯一生产模型。
- **服务方式**：生产优先使用 vLLM 暴露 OpenAI-compatible API，先采用 text-only、非思考模式、8K–16K 受控上下文、受控最大输出和 JSON Schema；云端硬件从单张 80GB GPU 的安全基线起测，再按实际显存和 P95 结果评估 48GB GPU 降本，不用官方 262K 上下文示例反推本项目必须多卡。
- **分层职责**：规则层继续权威处理安全和业务门控；云端开放权重模型承担 Scene/意图/轻情绪承接/槽位补全/结构化 JSON/普通 Stylist 对话/Memory proposal；输出低置信度、超时、合同失败或涉及复杂情绪和高歧义时才路由 CPA。
...
- **进入实现前验收口径**：在固定 30 条评测外新增至少 100 条真实中文对话、结构化合同、长短会话和情绪样例；同一云区、同一输入/输出上限对全部候选报告首 token、P50/P95、吞吐、单次与月度成本、JSON 合同通过率、人格/情绪承接盲评和安全指标。目标端到端 P95 `<=5s`、JSON 合同通过率 `>=99%`，并保持高急 Shopping Gate `100%`、Catalog `0`、衣物 ID 幻觉 `0`、硬约束违反 `0`。只有对照评测证明质量与安全达到门槛后，才允许提高云端开放权重模型路由占比；微调/LoRA 排在基线评测之后。
