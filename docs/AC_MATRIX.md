...
> 冻结日期：2026-08-04；S10 最终核验日期：2026-08-10。需求权威为 `docs/PRD.md`；本表用于委托、实现、审查和测试追踪。
...
| CNV-02/04/05/06/07 会话控制 | `/dialogue/turn` 保存 owner-bound Scene 与模式；每个新回合最多尝试一次 CPA，幂等重试不重复；服务端同步等待完整结果≤120s且业务超时不跨回合熔断；无任务聊天为 `stylist_chat+chat/scene=null`；轻情绪与明确任务并行，普通 chat 后任务可直接恢复；显式暂停锁存到显式 resume；30 分钟可收紧 TTL | 浏览器预算125s并先清空/显示经过秒数；等待中全入口防重入，reset取消旧请求；正式回复到达立即解锁，Trace 独立加载；所有续轮带同一 session；只在 `action=recommend` 渲染方向；支持/安全/关闭 0 方向、0 CTA；消息级严格显示 CPA 生成、本地回复或安全回应 | S9/S10 专项与全量 `203 passed`：真实“你好”15.970s、约会句38.057s；单POST、receipt0、近时表达/反例、情绪去重、owner/幂等/TTL、mode/action/control、Trace 与降级 | S5/S6/S7/S8/S9/S10 |
...
| SAFE-CPA 对话隐私与输出安全 | 每个新回合最多尝试一次 CPA；普通输入仅发送固定 Stylist 人格、必要画像 allowlist、最多 6 条截断历史、权威状态及脱敏当前文本；合法量体值只作为当前 session 的 `garment_fit_only` 上下文，原句/原值不进历史、长期记忆或 Trace；其他敏感/直接标识符只发送最小分类摘要。模型输出不是动作、工具、购物、ID、记忆或安全权威 | 严格 CPA 成功显示“CPA 生成”，一般 fallback 显示“本地回复”，真正 `safety_response` 显示“安全回应”；任何 ID、购物越权、量体原值复述、人物比例/身体/年龄/性吸引力评价、医疗、3D/视频均不展示 | 全量 `203 passed` + 独立攻击矩阵：单 `json|JSON` fence 窄兼容；非法 advisory 仅丢弃后继续全量正文校验；extra/action-control/购物/ID/person/medical/3D/video/fit echo 仍 fail-closed；输出拒绝和120s业务超时不毒化下一回合，网络/错模型仍可熔断；Trace 无正文/量体原值 | S3/M3/S5/S6/S7/S8/S9/S10 |
| PREV-04/05/06/08/10/12 Static2D 实验纵切 | `/preview/static-2d` 只接收 owner-bound 不可变 Look；服务端派生并重验衣物白名单/available/硬约束；CPA 固定 logical `grok4.5`、transport `grok-4.5-high`、reported allowlist `{grok-4.5-high,grok-4.5-build}`；身份资产不发送、identity=`not_assessed`；生成资产 owner/session 读取并可删除；3D/360°/视频 Provider 调用 0 | 仅在 Active Look 显示实验性 2D 入口；明确“中性无身份模特或平铺”、AI 标签和尺码/面料/垂坠局限；失败保留平铺/文字；无真人试穿、3D 或视频入口 | Static2D 自动化覆盖成功/降级与资源/安全生命周期；2026-08-10 真实 CPA smoke 确认 `grok-4.5-high` 不支持图像端点，0.229s 诚实降级且未切换模型 | S6/S10（实验性 R2，不计入 R1 DoD） |
...
| R1/S6–S10 文档一致性 | `/health`、Trace、报告版本、logical `grok4.5`、transport `grok-4.5-high`、精确 allowlist `{grok-4.5-high,grok-4.5-build}`、120/125s预算、对话/量体/近时边界与 Static2D 路由和实现一致 | 实验性 Static2D 与完整 R2 明确区分；第二成员、真人身份试穿、真实购物、3D/360°/视频无可用入口 | README/PRD/API/BUILD 合同与 UI 字符串审查；全量 pytest `203 passed`；validate/eval/Node/runtime/static/临时端口及真实8000 CPA smoke 全绿 | S4/M3/S6/S7/S8/S9/S10 |
